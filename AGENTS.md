...
| 优先级 | 事项 |
|---|---|
| **P0** | **daemon 的真实时段验证** —— 核心需求"按上课时段自动录制"目前只有单元测试覆盖，没在真实时间轴上跑过 |
| P1 | `cmd.rs` 里 **39 处带参数的文案**迁移（带 `{}` 插值的，不能正则批量处理） |
| P2 | **9 个内置插件本体**（宿主可用，范例见 `plugins/example-echo/`） |
| P3 | 需求外功能 `overlay`（桌面悬浮窗）的去留 —— **先问用户，不要自行删除** |
