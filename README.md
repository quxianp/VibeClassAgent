...
按课表在上课时段静默录屏录音（含**系统声音**与麦克风），在**时间表的午休与晚餐时段**
自动完成「语音转写 → 要点提取 → 截图关联 → 生成带图文档 → 推送到微信/QQ → 到期清理」全流程。
全程无提示、无弹窗、无托盘打扰。

处理时段**不用手工配置**：程序直接从时间表里挑出名字带「午休」「晚餐 / 放学后」的休息段。
用 DeepSeek 时还默认开启**错峰**——高峰时段（北京时间工作日 09:00–12:00 与
14:00–18:00，正好是上课时间）先把任务积压下来，等闲时（半价）再跑。

> **技术栈**：Rust 单语言，无 Python 运行时。
> **平台**：Windows 10 及以上。
> **测试**：225 个单元测试全绿 ｜ 界面冒烟 28 项全过 ｜ clippy 零告警 ｜ fmt 无差异。

> ### 📖 不会用？看 [`docs/使用手册.md`](docs/使用手册.md)
> 面向使用者的完整说明：第一次怎么配、每天怎么用、出问题怎么查。
> 五分钟能配完。

> ### 🖥️ 界面是主形态
> 双击「启动.cmd」打开图形界面（内嵌 HTTP 服务 + Edge 应用模式，无地址栏，
> 观感上就是个原生程序，目标机上不需要额外安装任何东西）。
> CLI 完整保留：`vca chat` 进交互式命令行，`vca --help` 看全部子命令。
...
| DeepSeek 错峰省钱 | ✅ | 高峰时段自动积压、闲时补跑（默认开，可关）；高峰 = 工作日 UTC 01–04 / 06–10 |
