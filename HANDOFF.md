...
核心需求"根据上课时段自动开启"目前只有单元测试覆盖。
构造课表让"现在"落在上课时段，跑 `vca run` 确认真的开始录制、结束后真的进处理流水线。
...
（路径格式已修正为 `bin/xxx.exe`）。宿主可用，`plugins/example-echo/` 是**可抄的完整范例**：
...
### P3：需求外功能 `overlay` 的去留（**待用户拍板**）

`overlay.rs`（1000+ 行）+ `tray.rs` 是**上一任 agent 自创的**：
原始需求里从没提过"桌面悬浮窗"，而它吃掉了平台层最长的文件。
CLI 里还挂着 `vca overlay` 命令。
